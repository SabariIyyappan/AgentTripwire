...
export const Route = createFileRoute("/reports/$id")({
  ...
  loader: ({ params }) => {
    const report = SAMPLE_REPORTS.find((r) => r.id === params.id);
...
function ReportDetail() {
  ...
  return (
    <SiteLayout>
      <SectionBlock>
        <Link to="/reports" className="mb-4 inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="h-3.5 w-3.5" /> All reports</Link>
        <GlassCard glow className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-2 inline-flex items-center gap-1.5 text-xs text-muted-foreground"><Clock className="h-3 w-3" /> {new Date(report.createdAt).toLocaleString()}</div>
            </div>
